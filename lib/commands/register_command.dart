...
Sets core.hooksPath to hooks/_ and writes thin shims that invoke compiled
binaries under .dart_tool/hooksman/executables/. User sources in hooks/*.dart
and hooks/*.sh are left untouched. Run register after clone.
''';
...
class RegisterCommand with PathsMixin {
  ...
  prepareExecutables(
    ...
    for (final hook in definedHooks.where((e) => e.isDart)) {
      // Dart import URIs always use `/`, even on Windows.
      final relativePath = fs.path
          .relative(hook.path, from: hooksDartToolDir.path)
          .replaceAll(r'\', '/');
...
void main(List<String> args) {
  ...
  ///
  /// Does **not** touch `.git/hooks`. Clears previous managed shims while
  /// preserving `.gitignore` / `README.md`.
...
''';
  ...
  /// @nodoc Kept for tests that still call the old name; prefers [writeShims].
  int copyExecutables(List<String> executables, {required String gitHooksDir}) {
    return writeShims(executables, managedHooksDir: gitHooksDir);
  }
  ...
  Future<int> run() async {
    ...
    if (hooksPath == null) {
      logger.err('Could not find hooks/_ directory path');
      return 1;
    }

    if (writeShims(executables, managedHooksDir: hooksPath) case final int code
        when code != 0) {
      return code;
    }
